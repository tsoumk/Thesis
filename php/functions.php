...
function getHTML($url){
	...
	curl_close($ch);
	if ( $http != 200 || empty($response) || strlen($response) < 1000 ){
...
function getNodes($url, $search){
	if ( !empty($url) ){
		$html = getHTML($url);
		if ( !empty($html) && $html !== false && $html != 404 ){
			$dom = new DOMDocument();
			$dom->loadHTML($html);
...
function entityExists($db, $tbl, $column, $value){
	if ( !empty($value) ){
		$sql = "SELECT EXISTS(SELECT 1 FROM $tbl WHERE $column = ?)";
		$stmt = $db->prepare($sql);
